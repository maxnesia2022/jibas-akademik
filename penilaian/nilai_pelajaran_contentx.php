...
$sql="SELECT p.nama, p.replid AS pelajaran, a.dasarpenilaian, j.jenisujian, j.replid AS jenis FROM aturannhb a, pelajaran p, jenisujian j WHERE a.replid='$idaturan' AND p.replid = a.idpelajaran AND a.idjenisujian = j.replid";
$result=QueryDb($sql);
...
if ($op=="jfd84rkj843h834jjduw3"){
	$sql_hapus_ujian="DELETE FROM ujian WHERE replid='$_REQUEST[replid]'";
	$result_hapus_ujian=QueryDb($sql_hapus_ujian);
	$sql_hapus_nau="DELETE FROM nau WHERE idaturan='$_REQUEST[idaturan]'";
	$result_hapus_nau=QueryDb($sql_hapus_nau);
	$sql_hapus_rataus="DELETE FROM rataus ".
...
$x=1;
	$sql_get_ujian="SELECT * FROM ujian WHERE idpelajaran='$pelajaran' AND idjenis='$jenis' AND idkelas='$kelas' AND idsemester='$semester' AND idaturan='$idaturan' ORDER by tanggal ASC";
	//echo $sql_get_ujian."<br>";
	$result_get_ujian=QueryDb($sql_get_ujian);
	//$i=1;
	while ($row_get_ujian=@mysqli_fetch_array($result_get_ujian)){
	$sql_hapus_bobotnau="DELETE FROM infobobotnau WHERE idujian='$row_get_ujian[replid]'";
	...
	if ($_REQUEST['bobot'.$x]<>""){
		...
		$sql_insert_info_bobot="INSERT INTO infobobotnau SET idujian='$idujian',idaturan='$idaturan'";
		$result_insert_info_bobot=QueryDb($sql_insert_info_bobot);
		
		$sql_get_last_id_info="SELECT LAST_INSERT_ID(replid) FROM infobobotnau ORDER BY replid DESC LIMIT 1";
		...
		$sql_insert_bobot="INSERT INTO bobotnau ".
	...
	$sql_get_nis_siswa="SELECT nis FROM siswa WHERE idkelas='$kelas'";
	...
	//Ambil idujian
	$sql_get_ujian="SELECT replid FROM ujian WHERE idpelajaran='$pelajaran' AND idkelas='$kelas' AND idsemester='$semester' AND idjenis='$jenis_penilaian' AND idaturan='$idaturan'";
	//echo $sql_get_ujian."<br>";
	$result_get_ujian=QueryDb($sql_get_ujian);
	$ujian_culip=0;
	while ($row_get_ujian=@mysqli_fetch_array($result_get_ujian)){
	//Ambil info bobot
	$sql_get_info_bobot="SELECT replid FROM infobobotnau WHERE idujian='$row_get_ujian[replid]'";
	...
	//Ambil bobot
	$sql_get_bobot="SELECT bobot FROM bobotnau WHERE idinfo='$row_get_info_bobot[replid]'";
	//echo $sql_get_bobot."<br>";
	$result_get_bobot=QueryDb($sql_get_bobot);
	$row_get_bobot=@mysqli_fetch_array($result_get_bobot);
	//Ambil nilai ujian
	$sql_get_nilai="SELECT nilaiujian FROM nilaiujian WHERE idujian='$row_get_ujian[replid]' AND nis='$row_get_nis_siswa[nis]'";
	...
	$sql_hapus_nau="DELETE FROM nau WHERE nis='$row_get_nis_siswa[nis]' AND idkelas='$kelas' AND idsemester='$semester' AND idjenis='$jenis_penilaian' AND idaturan='$idaturan'";
	//echo $sql_hapus_nau."<br>";
	$result_hapus_nau=QueryDb($sql_hapus_nau);
	
	//Insert ke tabel nau
	$sql_insert_nau="INSERT INTO nau SET nis='$row_get_nis_siswa[nis]',idkelas='$kelas',idsemester='$semester',idjenis='$jenis_penilaian',idpelajaran='$pelajaran',nilaiAU='$ratabulat',idaturan='$idaturan'";
...
if ($op=="osdiui4903i03j490dj"){
	$sql_hapus_nau="DELETE FROM nau WHERE idpelajaran='$pelajaran' AND idkelas='$kelas' AND idsemester='$semester' AND idjenis='$jenis_penilaian' AND idaturan='$idaturan'";
	$result_hapus_nau=QueryDb($sql_hapus_nau);
	if ($result_hapus_nau){
	?>
    <script language="JavaScript">
...
<tr>
    ...
    <table width="100%" border="0" height="100%">
   	<tr>
        ...
        <br />
	<?
    $sql_cek_ujian = "SELECT * FROM ujian WHERE idaturan='$idaturan' AND idkelas='$kelas' AND idsemester='$semester' ORDER by tanggal ASC";
...
<? $sql_get_nau_per_kelas="SELECT nilaiAU,keterangan FROM nau WHERE idkelas='$kelas' AND idsemester='$semester' AND idaturan='$idaturan'";
...
<?	$sql_siswa="SELECT * FROM siswa WHERE idkelas='$kelas' AND aktif=1 ORDER BY nama ASC";
...
?>
	...
	<?          
        ...
        for ($j=1;$j<=count($idujian);$j++) {		
            $sql_cek_nilai_ujian="SELECT * FROM nilaiujian WHERE idujian='$idujian[$j]' AND nis='$row_siswa[nis]'";
    ...
    ?>
    		...
    		<td align="center">
	<?	$sql_get_nau_per_nis="SELECT nilaiAU,replid,keterangan FROM nau WHERE nis='$row_siswa[nis]' AND idkelas='$kelas' AND idsemester='$semester' AND idaturan='$idaturan' ";
...
<?  $sql_cek_ujian_ratakelas="SELECT replid FROM ujian WHERE idkelas='$kelas' AND idsemester='$semester' AND idaturan='$idaturan' ORDER by tanggal ASC";
	...
	while ($row_cek_ujian_ratakelas=@mysqli_fetch_array($result_cek_ujian_ratakelas)){
		$sql_hitung_ratakelas="SELECT SUM(nilaiujian) as nilaiujian, COUNT(*) as jumlahnya FROM nilaiujian WHERE idujian='$row_cek_ujian_ratakelas[replid]'";
    	...
    	<table id="table" class="tab" width="50%" border="1">
			<tr>
				...
				<td width="15%" class="header" align="center" height="30">Bobot</td>
                 ...
                 <?
	$sql_cek_ujian="SELECT * FROM ujian WHERE idkelas='$kelas' AND idsemester='$semester' AND idaturan='$idaturan' ORDER by tanggal ASC";
	$result_cek_ujian=QueryDb($sql_cek_ujian);
	$ibobot=1;
	while ($row_cek_ujian=@mysqli_fetch_array($result_cek_ujian)){
	$sql_get_bobotnya="SELECT b.bobot FROM infobobotnau i, bobotnau b WHERE i.idujian='$row_cek_ujian[replid]' AND b.idinfo=i.replid";

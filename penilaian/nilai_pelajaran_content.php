...
if ($op == "jfd84rkj843h834jjduw3")
{
	...
	$sql_hapus_nau = "DELETE FROM nau WHERE idaturan='$_REQUEST[idaturan]' AND idkelas = '$kelas' AND idsemester = '$semester'";
	QueryDbTrans($sql_hapus_nau, $success);
	
	if ($success)
	{
		$sql_hapus_nilai_ujian = "DELETE FROM nilaiujian WHERE idujian='$_REQUEST[replid]'";	
	...
	if ($success)
	{
		$sql_hapus_ratauk = "DELETE FROM ratauk WHERE idujian = '$_REQUEST[replid]' AND idkelas = '$kelas' AND idsemester = '$semester'";
...
<?      $sql_siswa = "SELECT * FROM siswa WHERE idkelas='$kelas' AND aktif=1 ORDER BY nama ASC";
		...
		<?  } ?>
    		...
    		<td align="center">
	<?					
			$sql_get_nau_per_nis="SELECT nilaiAU,replid,keterangan,info1 FROM nau WHERE nis='$row_siswa[nis]' AND idkelas='$kelas' AND idsemester='$semester' AND idaturan='$idaturan'";
...
<?				} 
  ...
  <?
	...
	<? 	} ?>
   	...
   	<?					
		$sql_get_nau_per_nis="SELECT SUM(nilaiAU)/$jumsiswa FROM nau WHERE idkelas='$kelas' AND idsemester='$semester' AND idaturan='$idaturan'";
...
<? 			$sql_cek_ujian = "SELECT *, u.replid as replid 
			...
			while ($row_cek_ujian = @mysqli_fetch_array($result_cek_ujian))
			{
				...
				$sql_get_bobotnya = "SELECT b.replid, b.bobot FROM bobotnau b WHERE b.idujian='$row_cek_ujian[replid]'";								

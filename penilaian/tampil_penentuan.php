...
OpenDb();
if ($op == "del") {
	$sql = "SELECT replid FROM infonap WHERE idpelajaran = '$pelajaran' AND idsemester = '$semester' AND idkelas = '$kelas' ";
	...
	if ($num > 0) {
		...
		$sql = "DELETE FROM nap WHERE idinfo = '$row[replid]'";
		$result = QueryDb($sql) or die(mysqli_error());
		
		$sql = "DELETE FROM komennap WHERE idinfo = '$row[replid]'";
		$result = QueryDb($sql) or die(mysqli_error());
		
		$sql = "DELETE FROM infonap WHERE replid = '$row[replid]'";
		$result = QueryDb($sql) or die(mysqli_error());
	}
}
?>
...
$query_p = "SELECT nama FROM pelajaran WHERE replid = '$pelajaran'";
$result_p = QueryDb($query_p);
$row_p = @mysqli_fetch_array($result_p);

$query_infonap = "SELECT replid FROM infonap " .
...
$query_cek = "SELECT replid, nilaimin FROM infonap ".
...
if($num_cek > 0) {
    ...
    <tr>
        ...
        <?
        $query_ju = "SELECT replid, jenisujian FROM jenisujian WHERE idpelajaran = '$pelajaran'";
		...
		if($r_aturan != 0){            
		   ...
		   $t++;
				$query_nhb = "SELECT bobot ".
							 "FROM aturannhb WHERE aturannhb.replid = '$id_aturan'";
				$result_nhb = QueryDb($query_nhb) or die(mysqli_error());
				...
				while($row_nhb = @mysqli_fetch_array($result_nhb)) {
					...
					if($plit != "") {
						foreach($plit as $pl) {
							...
							$query_nau = "SELECT nau.nilaiAU FROM nau WHERE nis  = '$ns' ".
                                                                     "AND idjenis = '$ujian'" .
                                                                     "AND idkelas = '$kelas'  ". 
        							     "AND idpelajaran = '$pelajaran' ". 
                                                                     "AND idsemester = '$semester'";
                ...
                $query_nap = "SELECT nap.nilaihuruf FROM nap WHERE nis = '$ns' ".
...
if(isset($_POST['simpan'])) {
    $query_p = "INSERT INTO infonap(idpelajaran, idsemester, idkelas,  nilaimin) ".
                "VALUES('$_POST[pelajaran]', '$_POST[semester]', '$_POST[kelas]', '$_POST[nlulus]')";
    $result_p = QueryDb($query_p) or die(mysqli_error());

    $query_l = "SELECT LAST_INSERT_ID(replid) As replid FROM infonap ORDER BY replid DESC LIMIT 1";
    ...
    if(isset($_POST['nis'])) {
      $i = 0;
       foreach($_POST['nis'] as $key => $value) {
            $i++;
            $p = "predikat$i";
            $query_komen = "INSERT INTO komennap(nis, idinfo, predikat) ".
		   ...
		   echo "$query_komen<br>";
            ...
            for($x=1;$x<=$_POST['num_nhb'];$x++) {
                ...
                $query_nap = "INSERT INTO nap(nis, idaturan, idinfo, nilaiangka, nilaihuruf) ".

...
if(isset($_POST['simpan'])){
	...
	while($_POST['num_data'] >= $j){
		...
		$query_cek = "SELECT nau.replid FROM nau WHERE nau.idjenis = '$jenis_penilaian' AND idsemester = '$semester' AND idkelas = '$kelas' AND nis = '$_POST[$n]' AND idpelajaran = '$pelajaran'";
		//echo $query_cek;
		//exit();
		...
		if($num_cek == 0){
				$query_nau = "INSERT INTO nau (idpelajaran, nis, idkelas, idsemester, idjenis, nilaiAU, keterangan) ".
							 "VALUES ('$pelajaran','$_POST[$n]','$kelas','$semester','$jenis_penilaian','$_POST[$nau]','$ket')";
				$result_nau = QueryDb($query_nau) or die (mysqli_error());
		}elseif($num_cek  > 0){
				$query_nau = "UPDATE nau SET nau.nilaiAU = '$_POST[$nau]', idsemester = '$semester' WHERE nau.nis = '$_POST[$n]' AND idjenis = '$jenis_penilaian'";
				$result_nau = QueryDb($query_nau) or die (mysqli_error());
		}

...
class RecountNilaiRapor
{
	...
	{
		$sql = "SELECT replid FROM infonap 
	...
	private function GetAturanInfo()
	{
		...
		$sql = "SELECT a.replid FROM aturannhb a, kelas k 
	...
	private function DoRecount()
	{
		...
		// -- Ambil data siswa
		$sql = "SELECT nis FROM siswa WHERE idkelas='$this->idkelas' AND aktif=1 ORDER BY nama";
		$res = QueryDb($sql);
		...
		while ($success && ($row = mysqli_fetch_row($res)))
		{
			...
			{
				$sql = "SELECT n.nilaiau FROM nau n, aturannhb a 
			...
			$sql = "UPDATE nap SET nilaiangka = '$nilakhirpk', nilaihuruf = '$gradepk' 

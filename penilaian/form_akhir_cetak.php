...
$sql = "SELECT k.kelas AS namakelas, s.semester AS namasemester, a.tahunajaran, 
		  ...
		  FROM kelas k, semester s, tahunajaran a, pelajaran l, tingkat t, aturannhb n, jenisujian j, pegawai p 
...
<?	$sql_get_nis = "SELECT nis,nama,aktif,idkelas
    ...
    <?
        for ($i=1;$i<=count($idujian);$i++) {				
            $sql_get_nilai="SELECT n.nilaiujian FROM nilaiujian n WHERE n.nis='$row_get_nis[0]' AND idujian = $idujian[$i]";

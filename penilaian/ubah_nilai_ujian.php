...
OpenDb();
$query = "SELECT n.idujian, n.nilaiujian, n.keterangan, u.idaturan, u.idkelas, u.idsemester, s.nama, s.nis FROM ujian u, siswa s, nilaiujian n WHERE n.replid = '$id' AND n.idujian = u.replid AND s.nis = n.nis";
...
if(isset($_REQUEST["ubah"])) 
{
	BeginTrans();
	$success = 0;	
	$query_del_nau = "UPDATE nau SET nilaiAU = 0 WHERE idkelas='$idkelas' AND idsemester='$idsemester' AND idaturan='$idaturan' AND nis = '$nis'";
 	QueryDbTrans($query_del_nau, $success);

	if ($success)
	{
		$query = "SELECT replid FROM nilaiujian WHERE nis='$nis' AND idujian='$idujian'";
		$res = QueryDb($query);
		...
		$query = "INSERT INTO auditnilai SET jenisnilai='nilaiujian', idnilai='$idnilai', nasli='$nasli', nubah='$nilai', alasan='$alasan', pengguna='$pengguna', informasi='$info'";
		QueryDbTrans($query,$success);
	}

	if ($success)
	{
		$query = "UPDATE nilaiujian SET keterangan='$keterangan', nilaiujian='$nilai' WHERE nis='$nis' AND idujian='$idujian'";
		QueryDbTrans($query,$success);
	}

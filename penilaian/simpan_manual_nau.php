...
$sql_cek_nau="SELECT * FROM nau WHERE idpelajaran='$pelajaran' AND idkelas='$kelas' AND idsemester='$semester' AND idjenis='$jenis_penilaian' AND nis='$nis' AND replid='$idnau'";
//echo $sql_cek_nau;
...
if (@mysqli_num_rows($result_cek_nau)>0){
	...
	$sql_simpan="UPDATE nau SET nilaiAU='$nau' WHERE replid='$idnau'";
	} else {
	$sql_simpan="UPDATE nau SET nilaiAU='$nau',keterangan='Manual' WHERE replid='$idnau'";
	}
} else {
	$sql_simpan="INSERT INTO nau SET nilaiAU='$nau',idpelajaran='$pelajaran',idkelas='$kelas',idsemester='$semester',idjenis='$jenis_penilaian',nis='$nis',keterangan='Manual',idaturan='$idaturan' ";
}

...
while ($success && ($row_get_nis_siswa = @mysqli_fetch_array($result_get_nis_siswa)))
{
	...
	{	
		...
		{	
			...
			//Ambil bobot
			$sql = "SELECT bobot FROM bobotnau WHERE idujian = '$idujian'";
			$result_get_bobot = QueryDb($sql);
			$row_get_bobot = @mysqli_fetch_array($result_get_bobot);
			$b = (float)$row_get_bobot['bobot'];
			
			//Ambil nilai ujian
			$sql = "SELECT nilaiujian FROM nilaiujian WHERE idujian = '$idujian' AND nis = '$nis'";
		...
		{	
			...
			$sql_insert_nau = "UPDATE nau SET nilaiAU = '$ratabulat' WHERE replid = '$id_nau'";
		}
		else
		{
			$sql_insert_nau = 
	...
	else 
	{
		...
		{			
			...
			{
				if ($value[1]) 
					$sql_insert_nau = "UPDATE nau SET nilaiAU = '$value[0]', keterangan = 'Manual' WHERE replid = '$value[1]'";
				else 
					$sql_insert_nau = 
						"INSERT INTO nau 

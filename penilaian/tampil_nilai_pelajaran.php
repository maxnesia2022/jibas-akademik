...
<input type="hidden" name="dasar_penilaian" value="<?=$dasar_penilaian ?>">
	...
	<?
	$sql_jenisujian="SELECT * FROM jenisujian WHERE replid='$jenis_penilaian'";
	...
	<table width="100%" id="table" class="tab" border="1">
		<tr >
			...
			while($row_qz = @mysqli_fetch_array($result_qz)){
				$z++;
				$sql = "SELECT count(*) as cnt FROM nilaiujian WHERE idujian = '$row_qz[replid]' GROUP BY nis, nilaiujian HAVING cnt > 1";
				$rs = QueryDb($sql);
			...
			<td class="headerlong" align="center" height="30">Rata-Rata Siswa</td>
					<?
					 $query_ju = "SELECT * FROM jenisujian ".
			...
			*/
			...
			<br><br>
				...
				<tr>
					...
					$query_nuj = "SELECT nilaiujian FROM nilaiujian WHERE idujian = '$row_qz[replid]'";
	...
	if($_POST['rtn'] == "1"){		//jika checklist pertama yg dipilih (rata-rata nilai)
		...
		while($i <= $_POST['num_qz']){
			...
			if(isset($_POST[$ruj])){
				...
				$query_iduj = "SELECT * FROM nilaiujian WHERE nilaiujian.idujian = '$_POST[$ruj]'";
		...
		foreach($data_uj as $ns => $v) {
			//query buat cek data udah ada belon untuk data jenis penilaian. Klo belum insert, klo udah di update
			 $query_cek = "SELECT nau.replid, nau.idjenis FROM nau WHERE nau.idjenis = '$jenis_penilaian' AND idsemester = '$semester' AND idkelas = '$kelas' AND nis = '$ns' AND idpelajaran = '$pelajaran'";
			$result_cek = QueryDb($query_cek);
			$num_cek = mysqli_num_rows($result_cek);
			$row_cek = mysqli_fetch_array($result_cek); 
		
			$rata=$v/$ttl_bbt;
			$query_id = "SELECT nau.replid FROM nau WHERE nau.idjenis = '$jenis_penilaian' AND idsemester = '$semester' AND idkelas = '$kelas' AND nis = '$_POST[$ns]' AND idpelajaran = '$pelajaran'";
			$result_id = QueryDb($query_id);
			$row_id = @mysqli_fetch_array($result_id);
			if($num_cek == 0){
					$query_nau = "INSERT INTO nau (idpelajaran, nis, idkelas, idsemester, idjenis, nilaiAU) ".
								 "VALUES ('$pelajaran','$ns','$kelas','$semester','$jenis_penilaian','$rata')";
					$result_nau = QueryDb($query_nau) or die(mysqli_error());		
			}elseif($num_cek > 0){
					$query_nau = "UPDATE nau SET ".
		...
		if($num_cek == 0){
				echo $query_inf = "INSERT INTO infobobotujian ".
				...
				$query_inf = "UPDATE infobobotujian SET info = '$inf' WHERE idjenisujian = '$jenis_penilaian'";
				$result_inf = QueryDb($query_inf) or die (mysqli_error());
		}
...
elseif($_POST['rtn'] == "2"){			//jika checklist kedua yg dipilih
	...
	if($_POST['jum_jns'] < $_POST['t_max']){
		...
		$query_cek = "SELECT nau.replid, nau.idjenis FROM nau WHERE nau.idjenis = '$jenis_penilaian' ";
		$result_cek = QueryDb($query_cek);
		$num_cek = mysqli_num_rows($result_cek);
		$row_cek = mysqli_fetch_array($result_cek); 

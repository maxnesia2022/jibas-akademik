...
$sql_ta="SELECT * FROM tahunajaran WHERE replid='$tahunajaran'";
$result_ta=QueryDb($sql_ta);
$row_ta=@mysqli_fetch_array($result_ta);
$tglawal=$row_ta['tglmulai'];
$tglakhir=$row_ta['tglakhir'];	

$sql_get_siswa="SELECT nis,nama FROM siswa WHERE idkelas='$kelas' AND aktif = 1 ORDER BY nama";
$result_get_siswa=QueryDb($sql_get_siswa);

$sql_get_siswa1="SELECT nis,nama FROM siswa WHERE idkelas='$kelas' AND aktif = 1 ORDER BY nama";
$result_get_siswa1=QueryDb($sql_get_siswa1);

$sql = "SELECT k.kelas AS namakelas, s.semester AS namasemester, a.tahunajaran, t.tingkat, l.nama, a.departemen FROM kelas k, semester s, tahunajaran a, tingkat t, pelajaran l WHERE k.replid = $kelas AND s.replid = $semester AND a.replid = $tahunajaran AND k.idtahunajaran = a.replid AND k.idtingkat = t.replid";// AND l.replid = $pelajaran";
$result = QueryDb($sql);
$row = @mysqli_fetch_array($result);
	
$sql_get_w_kls="SELECT p.nama as namawalikelas, p.nip as nipwalikelas FROM pegawai p, kelas k WHERE k.replid='$kelas' AND k.nipwali=p.nip";
$rslt_get_w_kls=QueryDb($sql_get_w_kls);
$row_get_w_kls=@mysqli_fetch_array($rslt_get_w_kls);
	
$sql_get_kepsek="SELECT d.nipkepsek as nipkepsek,p.nama as namakepsek FROM pegawai p, departemen d WHERE  p.nip=d.nipkepsek AND d.departemen='$departemen'";
//echo $sql_get_kepsek;

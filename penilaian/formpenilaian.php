...
<?			$sql = "SELECT DISTINCT p.nip,p.nama FROM pegawai p, guru g, pelajaran l WHERE p.nip = g.nip AND g.idpelajaran = '$pelajaran' AND g.aktif = 1  ORDER BY p.nama  ";
			$result = QueryDb($sql);
			while ($row = @mysqli_fetch_array($result)) 
			{
				if ($nip == "") 				
...
<?					
					break;
			case 2 :
				$sql1="SELECT * FROM aturannhb WHERE idtingkat='$tingkat' AND idpelajaran='$pelajaran' AND aktif=1 AND nipguru='$nip'";

...
for ($i=1;$i<=$jum;$i++) {	
	...
	if ($departemen<>"-1" && $angkatan>0) {	
		$query1 = "SELECT COUNT(*) As Jum$field FROM siswa s, angkatan a WHERE s.idangkatan='$angkatan' AND a.replid=s.idangkatan AND a.departemen='$departemen' AND s.aktif = $filter ";
	}
...
if ($departemen=="-1") {
	if ($angkatan < 0){
		$query2 = "SELECT angkatan FROM angkatan";
	} else {
		$query2 = "SELECT angkatan FROM angkatan WHERE replid = '$angkatan'";
	}
} else {		
	if ($angkatan < 0){
		$query2 = "SELECT angkatan FROM angkatan WHERE departemen='$departemen'";
	} else {
		$query2 = "SELECT angkatan FROM angkatan WHERE replid = '$angkatan' AND departemen='$departemen' ";
	}
}

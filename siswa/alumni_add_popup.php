...
if(isset($_REQUEST["simpan"])){
	...
	BeginTrans();
	$success=0;
	$sql="SELECT s.nis as nis, s.idkelas as idkelas, t.departemen as departemen, t.replid as tingkat FROM siswa s, kelas k, tingkat t WHERE s.nis='$_REQUEST[nis]' AND k.replid=s.idkelas AND k.idtingkat=t.replid";
	
	$result=QueryDbTrans($sql, $success);
	$row=@mysqli_fetch_array($result);
	if ($success){
		$sql="INSERT INTO alumni SET nis='$row[nis]',klsakhir='$row[idkelas]',tktakhir='$row[tingkat]',tgllulus='$tgl',keterangan='".CQ($_REQUEST['keterangan'])."'";
		//echo $sql;
		//exit;
		QueryDbTrans($sql, $success);
		}
	if ($success){
		$sql="UPDATE riwayatkelassiswa SET aktif=0 WHERE nis='$_REQUEST[nis]' AND aktif=1";
		QueryDbTrans($sql, $success);
		//echo $sql;
		//exit;
		}
	if ($success){
		$sql="UPDATE riwayatdeptsiswa SET aktif=0 WHERE nis='$_REQUEST[nis]' AND aktif=1";
		QueryDbTrans($sql, $success);
		}
	if ($success){
		$sql="UPDATE siswa SET aktif=0, alumni=1 WHERE nis='$_REQUEST[nis]'";
		QueryDbTrans($sql, $success);
		}
	if ($success){
		CommitTrans(); 

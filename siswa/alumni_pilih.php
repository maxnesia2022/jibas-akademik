...
if (isset($_REQUEST['alumnikan']))
{
	...
	for ($ialumni = 1; $success && $ialumni <= $jumalumni; $ialumni++)
	{
		...
		if ($nis && $cek)
		{
			...
			$sql_siswa = "UPDATE siswa SET aktif=0, alumni=1 WHERE nis='$nis'";
			QueryDbTrans($sql_siswa, $success);

			if ($success)
			{	
				$sql_kelas = "UPDATE riwayatkelassiswa
								 SET aktif=0
							   WHERE nis='$nis'
							     AND aktif=1";
			...
			if ($success)
			{
				$sql_dept = "UPDATE riwayatdeptsiswa
				                SET aktif = 0
						  	  WHERE nis='$nis'
							    AND aktif=1";

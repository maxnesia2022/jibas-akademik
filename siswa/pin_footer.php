...
if ($op == "dw8dxn8w9ms8zs22") {
	$pin = random(5);
	OpenDb();
	$sql	= "SELECT pinsiswa,pinortu FROM siswa WHERE nis='$_REQUEST[nis]'";
	$result = QueryDb($sql);
	...
	$sql = "UPDATE siswa SET $_REQUEST[field] = '$pin' WHERE nis = '$_REQUEST[nis]'";
	QueryDb($sql);
	CloseDb();
}
...
<? 	
	OpenDb();
	$sql = "SELECT * FROM siswa s WHERE s.idkelas = '$kelas' AND s.aktif = 1 ORDER BY $urut $urutan ";

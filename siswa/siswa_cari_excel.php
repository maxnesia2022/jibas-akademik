...
OpenDb();
$sql = "SELECT s.replid,s.nis,s.nama,s.asalsekolah,s.tmplahir,DATE_FORMAT(s.tgllahir,'%d %M %Y') as tgllahir,s.status,t.tingkat,k.kelas,s.aktif,s.nisn from siswa s, kelas k, tingkat t WHERE s.".$jenis." LIKE '%$cari%' AND k.replid=s.idkelas AND k.idtingkat=t.replid AND t.departemen='$departemen' ORDER BY $urut $urutan";
$result=QueryDb($sql);
	...
	<td align="center"><?
		if ($row_siswa['aktif']==1){
			...
			if ($row_siswa['alumni']==1){
				$sql_get_al="SELECT DATE_FORMAT(a.tgllulus, '%d %M %Y') as tgllulus FROM alumni a WHERE a.nis='$row_siswa[nis]'";	
			...
			if ($row_siswa['statusmutasi']!=NULL){
				$sql_get_mut="SELECT DATE_FORMAT(m.tglmutasi, '%d %M %Y') as tglmutasi,j.jenismutasi FROM jenismutasi j, mutasisiswa m WHERE j.replid='$row_siswa[statusmutasi]' AND m.nis='$row_siswa[nis]' AND j.replid=m.jenismutasi";	

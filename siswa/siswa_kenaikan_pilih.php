...
if ($op=="x2378e23dkofh73n25ki9234"){
	OpenDb();
	$sql_kap_kelas_tujuan="SELECT kapasitas FROM kelas WHERE replid=$kelastujuan";
	...
	$sql_jum_kelas_tujuan="SELECT COUNT(nis) FROM siswa WHERE idkelas=$kelastujuan AND aktif = 1";
	...
	if ((int)$kap_kelas_tujuan <= (int)$row_jum_kelas_tujuan[0]){
		...
		BeginTrans();
		$success=0;
		$sql_naik="UPDATE siswa SET idkelas=$kelastujuan WHERE nis='$nisdipindah'";
		QueryDbTrans($sql_naik, $success);
		if ($success){
			$sql_naik_kelas_update="UPDATE riwayatkelassiswa SET aktif=0 WHERE nis='$nisdipindah' AND idkelas='$kelas'";
			QueryDbTrans($sql_naik_kelas_update, $success);
		}
		
		if ($success){
			$sql_naik_kelas_insert="INSERT INTO riwayatkelassiswa SET idkelas=$kelastujuan, aktif=1, nis='$nisdipindah' ,mulai='$sekarang',status=1,keterangan='$ket'";
...
<? 	if ($jenis <> ""){ 
		OpenDb();
		$sql_tot = "SELECT s.nis,s.nama,s.idkelas,k.kelas,s.replid,t.tingkat FROM siswa s, kelas k, tingkat t WHERE $string s.idkelas = k.replid AND k.idtahunajaran = '$tahunajaran' AND s.aktif=1 AND k.idtingkat = t.replid AND t.replid = '$tingkat'"; 
		//echo $sql_tot;
		...
		$sql_siswa = "SELECT s.nis,s.nama,s.idkelas,k.kelas,s.replid,t.tingkat FROM siswa s, kelas k, tingkat t WHERE $string s.idkelas = k.replid AND k.idtahunajaran = '$tahunajaran' AND s.aktif=1 AND k.idtingkat = t.replid AND t.replid = '$tingkat' ORDER BY $urut $urutan LIMIT ".(int)$page*(int)$varbaris.",$varbaris";
...
<?		if ($page==0)
		...
		while ($row_siswa=@mysqli_fetch_row($result_siswa)){
            $sql_kelas="SELECT replid,kelas FROM kelas WHERE replid='$row_siswa[2]'";

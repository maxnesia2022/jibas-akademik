...
if (isset($_REQUEST['Simpan'])) {
	OpenDb();
	$sql = "SELECT * FROM semester WHERE semester = '$semester' AND departemen = '$departemen'";
	$result = QueryDb($sql);
	...
	if (mysqli_num_rows($result) > 0) {
		...
		$sql = "INSERT INTO semester SET semester='$semester',departemen='$departemen',keterangan='$keterangan',aktif=1";
		$result = QueryDb($sql);
		$sql = "SELECT replid FROM semester WHERE departemen='$departemen' ORDER BY replid DESC LIMIT 1";
		$result = QueryDb($sql);
		$row = @mysqli_fetch_row($result);
		$sql = "UPDATE semester SET aktif=0 WHERE departemen='$departemen' AND replid<>'$row[0]'";
		$result = QueryDb($sql);
		CloseDb();

...
if ((strlen($nama) > 0) && (strlen($nip) > 0))
	...
	$sql = "SELECT s.nis, s.nama, k.kelas FROM siswa s,kelas k ,tingkat ti,tahunajaran ta  WHERE s.nama LIKE '%$nama%' AND k.replid=s.idkelas AND s.statusmutasi=0  AND k.idtahunajaran=ta.replid AND k.idtingkat=ti.replid AND ta.departemen='$departemen' AND ti.departemen='$departemen' AND s.aktif=1 ORDER BY s.nama"; 
else if (strlen($nis) > 0)
	$sql = "SELECT s.nis, s.nama, k.kelas FROM siswa s,kelas k ,tingkat ti,tahunajaran ta  WHERE k.replid=s.idkelas AND s.nis LIKE '%$nis%' AND s.statusmutasi=0  AND k.idtahunajaran=ta.replid AND k.idtingkat=ti.replid AND ta.departemen='$departemen' AND ti.departemen='$departemen' AND s.aktif=1 ORDER BY s.nama"; 
else if ((strlen($nama) == 0) || (strlen($nis) == 0))
	$sql = "SELECT s.nis, s.nama, k.kelas FROM siswa s,kelas k,tingkat ti,tahunajaran ta WHERE k.replid=s.idkelas AND s.statusmutasi=0 AND k.idtahunajaran=ta.replid AND k.idtingkat=ti.replid AND ta.departemen='$departemen' AND ti.departemen='$departemen' AND s.aktif=1  ORDER BY s.nama"; 
$result = QueryDb($sql);
$cnt = 1;

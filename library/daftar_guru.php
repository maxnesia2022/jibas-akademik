...
<tr>
  ...
  <td><select name="pelajaran" id="pelajaran" onchange="get_guru()" <?=$disable?> style="width:250px;">
    ...
    <?
        $sql_pel="SELECT * FROM pelajaran pel WHERE pel.aktif=1 $sql_tambahdep ORDER BY pel.nama";
...
<tr>
	<td colspan="2" align="center">
        ...
        <? 
        OpenDb();
		$sql = "SELECT p.nip, p.nama, pel.replid, pel.departemen FROM pegawai p, guru g, pelajaran pel, departemen d WHERE g.nip=p.nip AND g.idpelajaran=pel.replid AND pel.departemen = d.departemen AND g.aktif = 1 $sql_tambahpel $sql_tambahdep GROUP BY p.nip ORDER BY p.nama";

...
if (isset($_REQUEST['Submit'])) { 
	...
	if ((strlen($nama) > 0) && (strlen($nip) > 0))
		$sql = "SELECT nip, nama FROM pegawai WHERE nama LIKE '%$nama%' AND nip LIKE '%$nip%' ORDER BY nama"; 
	else if (strlen($nama) > 0)
		$sql = "SELECT nip, nama FROM pegawai WHERE nama LIKE '%$nama%' ORDER BY nama"; 
	else if (strlen($nip) > 0)
		$sql = "SELECT nip, nama FROM pegawai WHERE nip LIKE '%$nip%' ORDER BY nama"; 

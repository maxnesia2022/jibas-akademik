...
if (isset($_REQUEST['submit']) || $_REQUEST['submit'] == 1) { 
	...
	if ((strlen($nama) > 0) && (strlen($nip) > 0))
		$sql = "SELECT p.nip, p.nama, pel.nama, pel.departemen FROM pegawai p, guru g, pelajaran pel, departemen d WHERE p.nama LIKE '%$nama%' AND p.nip LIKE '%$nip%' AND pel.replid=g.idpelajaran AND g.nip=p.nip AND pel.departemen = d.departemen GROUP BY p.nip ORDER BY d.urutan, p.nama"; 
	else if (strlen($nama) > 0)
		$sql = "SELECT p.nip, p.nama, pel.nama, pel.departemen FROM pegawai p, guru g, pelajaran pel, departemen d WHERE p.nama LIKE '%$nama%' AND pel.replid=g.idpelajaran AND g.nip=p.nip AND pel.departemen = d.departemen GROUP BY p.nip ORDER BY d.urutan, p.nama"; 
	else if (strlen($nip) > 0)
		$sql = "SELECT p.nip, p.nama, pel.nama, pel.departemen FROM pegawai p, guru g, pelajaran pel, departemen d WHERE p.nip LIKE '%$nip%' AND pel.replid=g.idpelajaran AND g.nip=p.nip AND pel.departemen = d.departemen GROUP BY p.nip ORDER BY p.nama"; 
	//echo $sql;	
	//else if ((strlen($nama) == 0) || (strlen($nip) == 0))
	//	$sql = "SELECT p.nip, p.nama, pel.nama, pel.departemen FROM pegawai p, guru g, pelajaran pel, departemen d AND pel.replid=g.idpelajaran AND g.nip=p.nip ORDER BY p.nama";	
	$result = QueryDb($sql);

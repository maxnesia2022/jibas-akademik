...
if ($op == "xm8r389xemx23xb2378e23") {
	...
	$sql1="UPDATE riwayatkelassiswa SET aktif=1 WHERE nis='$nis' AND idkelas = '$idkelas' ORDER BY mulai DESC LIMIT 1";
	$result1=QueryDbTrans($sql1, $success);
	
	if ($success){
		$sql1="UPDATE riwayatdeptsiswa SET aktif=1 WHERE nis='$nis' AND departemen='$departemen' ORDER BY mulai DESC LIMIT 1";
		$result1=QueryDbTrans($sql1, $success);
	}
	
	if ($success){
		$sql1="UPDATE siswa SET aktif=1, statusmutasi=NULL, alumni = 0 WHERE nis='$nis'";
		$result=QueryDbTrans($sql1, $success);
	}
	
	if ($success){
		$sql1="DELETE FROM alumni WHERE nis='$nis' AND departemen = '$departemen' AND klsakhir='$idkelas' AND tgllulus = '$tglmutasi'";
		$result1=QueryDbTrans($sql1, $success);
	}
	
	if ($success){
		$sql1="DELETE FROM mutasisiswa WHERE replid='$_REQUEST[replid]'";
		$result1=QueryDbTrans($sql1, $success);
	}

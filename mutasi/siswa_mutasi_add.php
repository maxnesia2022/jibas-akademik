...
if (isset($_REQUEST['Simpan'])) {
	...
	BeginTrans();
	$success=0;
	$sql="UPDATE siswa SET aktif=0, statusmutasi='$mutasi', alumni=1 WHERE nis='$nis'";
	QueryDbTrans($sql, $success);
		
	if ($success){
		$sql="UPDATE riwayatkelassiswa SET aktif=0 WHERE nis='$nis' AND aktif=1";
		QueryDbTrans($sql,$success);
	}

	if ($success) {
		$sql="UPDATE riwayatdeptsiswa SET aktif=0 WHERE nis='$nis' AND aktif=1";
		QueryDbTrans($sql,$success);
	}
	
	if ($success){
		$sql="INSERT INTO alumni SET nis='$nis', tgllulus='$tglmutasi', tktakhir='$idtingkat', klsakhir='$idkelas', departemen = '$departemen'";
		QueryDbTrans($sql,$success);
	}
	
	if ($success){
		$sql="INSERT INTO mutasisiswa SET nis='$nis', jenismutasi='$mutasi', tglmutasi='$tglmutasi', keterangan='$keterangan', departemen = '$departemen'";
		QueryDbTrans($sql, $success);
		//echo $sql; 
		//RollBackTrans();
		//exit;

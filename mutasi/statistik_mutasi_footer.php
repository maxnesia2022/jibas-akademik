...
for($a=$tahunawal;$a<=$tahunakhir;$a++){
	 	$query="SELECT COUNT(*) As Jum,j.jenismutasi As jenismutasi, j.replid as replidmutasi FROM mutasisiswa m,jenismutasi j,angkatan a,siswa s WHERE	a.departemen='$departemen' AND s.idangkatan=a.replid  AND m.nis=s.nis AND s.statusmutasi=m.jenismutasi AND m.jenismutasi=j.replid AND YEAR(m.tglmutasi) = '$a' GROUP BY jenismutasi";

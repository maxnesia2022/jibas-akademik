...
OpenDb();	
$query_mutasi="SELECT s.nis,s.nama,s.statusmutasi,j.jenismutasi,m.tglmutasi,m.keterangan,s.replid FROM siswa s, jenismutasi j,mutasisiswa m,kelas k,tahunajaran t WHERE t.departemen='$departemen' AND t.replid=k.idtahunajaran AND k.replid=s.idkelas AND j.replid=s.statusmutasi AND j.replid=m.jenismutasi AND s.nis=m.nis AND YEAR(m.tglmutasi)='$tahun' ORDER BY j.replid";

$query_mutasi="SELECT s.nis,s.nama,s.statusmutasi,j.jenismutasi,m.tglmutasi,m.keterangan,s.replid FROM siswa s, jenismutasi j,mutasisiswa m,kelas k,tahunajaran t WHERE t.departemen='$departemen' AND t.replid=k.idtahunajaran AND k.replid=s.idkelas AND j.replid=s.statusmutasi AND j.replid=m.jenismutasi AND s.nis=m.nis ORDER BY j.replid";
...
if ($op=="fdh6rt5dgh98rth658rg48ger"){ //Hapus siswa
	...
	BeginTrans();
	$success=0;
	$sql_hapus="DELETE FROM mutasisiswa WHERE nis='$_REQUEST[nis]'";
	QueryDbTrans($sql_hapus,$success);
	if ($success){
		$sql_hapus_rwyt_kelas="DELETE FROM riwayatkelassiswa WHERE nis='$_REQUEST[nis]'";
		QueryDbTrans($sql_hapus_rwyt_kelas,$success);
	}
	if ($success){
		$sql_hapus_rwyt_dep="DELETE FROM riwayatdeptsiswa WHERE nis='$_REQUEST[nis]'";
		QueryDbTrans($sql_hapus_rwyt_dep,$success);
	}
	if ($success){
		$sql_hapus_siswa="DELETE FROM siswa WHERE nis='$_REQUEST[nis]'";
...
if ($op=="jkds7o34jd98wejbf9uwe"){ //Aktifkan kembali
	...
	BeginTrans();
	$success=0;
	$sql_hapus_mutasi="DELETE FROM mutasisiswa WHERE nis='$_REQUEST[nis]'";
	QueryDbTrans($sql_hapus_mutasi,$success);
	if ($success){
		$sql_update_siswa="UPDATE siswa SET aktif=1 WHERE nis='$_REQUEST[nis]'";

...
openDb();
		$query="SELECT s.nis,s.nama,m.tglmutasi,m.keterangan FROM mutasisiswa m,jenismutasi j,angkatan a,siswa s WHERE	a.departemen='$departemen' AND s.idangkatan=a.replid  AND m.nis=s.nis AND s.statusmutasi=m.jenismutasi AND m.jenismutasi=j.replid AND YEAR(m.tglmutasi) = '$tahun' AND j.jenismutasi='$_REQUEST[jenismutasi]'" ;

...
if (isset($_REQUEST['Simpan'])) {
	OpenDb();
	$sql_cek= "SELECT * FROM statusguru WHERE status='$status' AND replid <>'$replid_baru'";
	...
	if (mysqli_num_rows($hasil) > 0) {
		...
		$ERROR_MSG = "Status guru $status sudah digunakan!";
   		...
   		$sql_update= "UPDATE statusguru SET status='$_REQUEST[status]',keterangan='$_REQUEST[keterangan]' WHERE replid='$replid_baru'";
...
OpenDb();
$sql_tampil="SELECT replid,status,keterangan from statusguru where replid='$replid'";

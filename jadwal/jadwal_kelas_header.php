...
{
	$sql_update_aktif = "UPDATE infojadwal SET aktif = '$_REQUEST[newaktif]' WHERE replid = '$_REQUEST[replid]' ";
	QueryDb($sql_update_aktif);
}
else if ($op=="fcjfootkpsmfkgjdmv")
{
	$sql_update_terlihat = "UPDATE infojadwal SET terlihat = '$_REQUEST[newvis]' WHERE replid = '$_REQUEST[replid]' ";
	QueryDb($sql_update_terlihat);
}
else if ($op=="xm8r389xemx23xb2378e23")
{
	$sql_delete = "DELETE FROM infojadwal WHERE replid = '$_REQUEST[info_jadwal]'";
...
<?          $sql_tahunajaran="SELECT * FROM tahunajaran WHERE departemen='$departemen' AND aktif=1 ORDER BY replid DESC";
...
<?          $sql_kelas="SELECT * FROM kelas WHERE idtahunajaran='$tahunajaran' AND idtingkat='$tingkat' ORDER BY kelas";
...
<?  		$sql_info_jadwal="SELECT * FROM infojadwal WHERE idtahunajaran = '$tahunajaran' AND aktif=1 ORDER BY aktif DESC";

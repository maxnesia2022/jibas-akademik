...
else if ($op == "vmt489tukd9fcmf92kd2309scm2323rc3")
{
	...
	if ($ncal > 1)
	{
		$sql = "UPDATE kalenderakademik SET aktif = 0 WHERE replid = '$_REQUEST[replid]'";	
		QueryDb($sql);
	}
...
<tr>	
    ...
    <tr>
		...
		if ($departemen <> "") {	
			OpenDb();
			$sql = "SELECT * FROM kalenderakademik i, tahunajaran t WHERE t.departemen ='$departemen' AND i.idtahunajaran = t.replid";
			//$sql = "SELECT COUNT(t.replid) FROM tahunajaran t WHERE t.departemen = '$departemen' UNION SELECT COUNT(k.replid) FROM kalenderakademik k WHERE k.departemen = '$departemen'";
			//echo $sql;
			$result = QueryDb($sql);
...
if ($departemen <> "") {	
	OpenDb();	
	$sql = "SELECT i.kalender, i.aktif, i.replid, i.terlihat, i.idtahunajaran, t.tglmulai, t.tglakhir FROM kalenderakademik i, tahunajaran t WHERE t.departemen ='$departemen' AND i.idtahunajaran = t.replid ORDER BY $urut $urutan";

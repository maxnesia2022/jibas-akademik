...
<tr>
    ...
    <tr>
    	...
    	<td><select name="kalender" id="kalender" onchange="change(0)" style="width:250px;" onkeypress="return focusNext('tabel', event)">
          <? OpenDb();
				$sql_kalender = "SELECT * FROM kalenderakademik where departemen='$departemen' ORDER BY aktif DESC, replid ASC";
   ...
   <tr>
		...
		if ($kalender <> "" ) {
			OpenDb();
			$sql = "SELECT * FROM tahunajaran t, kalenderakademik k WHERE t.replid=k.idtahunajaran AND k.replid = '$kalender'";
			$result = QueryDb($sql);

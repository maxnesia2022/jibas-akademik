...
{			
	...
	if ($num > 0)
	{
		...
		$sql = "SELECT ij.deskripsi, pg.nama, p.nama AS pelajaran, k.kelas, j.departemen, j.hari, j.jamke AS jam1, j.jamke + j.njam - 1 AS jam2 
				   FROM infojadwal ij, jadwal j, pegawai pg, pelajaran p, kelas k 
...
$sql1 = "SELECT tahunajaran FROM tahunajaran t WHERE replid = '$tahunajaran' ";
$result1 = QueryDb($sql1);
$row1 = mysqli_fetch_array($result1); 
$tahun = $row1['tahunajaran'];

$sql1 = "SELECT nama FROM pegawai WHERE nip = '$nip' ";
$result1 = QueryDb($sql1);
$row1 = mysqli_fetch_array($result1); 
$nama = $row1['nama'];

...
$sql_cek = "SELECT * FROM login WHERE login = '$_REQUEST[nip]'";
$res_cek = QueryDb($sql_cek);
$jum_cek = @mysqli_num_rows($res_cek);

$query_cek2 = "SELECT * FROM hakakses WHERE login = '$_REQUEST[nip]' AND modul='SIMAKA'";
...
if (isset($_REQUEST['simpan'])) {
	...
	$query_c = "SELECT * FROM hakakses WHERE login = '$_REQUEST[nip]' AND tingkat = $tingkat AND modul = 'SIMAKA' $sql_dep";
	$result_c = QueryDb($query_c);
    $num_c = @mysqli_num_rows($result_c);
	
	$query_cek = "SELECT * FROM login WHERE login = '$_REQUEST[nip]'";
	...
	if ($num_c == 0) {
		if ($tingkat==1){
			//Kalo manajer
			if ($num_cek == 0) {
				$sql_login="INSERT INTO login SET login='$_REQUEST[nip]', password='$pass', aktif=1";
				QueryDbTrans($sql_login, $success);		
			}		
				
			$sql_hakakses="INSERT INTO hakakses SET login='$_REQUEST[nip]', tingkat=1, modul='SIMAKA', keterangan='".CQ($_REQUEST['keterangan'])."'";
		} elseif ($tingkat==2){
			//Kalo staf
			if ($num_cek == 0) {
				$sql_login="INSERT INTO login SET login='$_REQUEST[nip]', password='$pass', aktif=1";
			...
			$sql_hakakses="INSERT INTO hakakses SET login='$_REQUEST[nip]', departemen='$_REQUEST[departemen]', tingkat=2, modul='SIMAKA', keterangan='".CQ($_REQUEST['keterangan'])."'";
		}
		if ($success)	

...
if (isset($_REQUEST['simpan'])) {
	...
	if ($login=='landlord' || $login=='LANDLORD'){
		OpenDb();
		$sql = "SELECT password FROM landlord WHERE password=md5('$_REQUEST[passlama]')";
		$result = QueryDb($sql);
		if (mysqli_num_rows($result) == 0) {
			CloseDb(); 
			$MYSQL_ERROR_MSG = "Password lama Anda tidak cocok!";
		} else {
			$sql = "UPDATE landlord SET password=md5('$_REQUEST[pass1]')";
			$result = QueryDb($sql);
			CloseDb();
		...
		OpenDb();
		$sql = "SELECT login FROM login WHERE password=md5('$_REQUEST[passlama]') AND login='$nip'";
		$result = QueryDb($sql);
		if (mysqli_num_rows($result) == 0) {
			CloseDb(); 
			$MYSQL_ERROR_MSG = "Password lama Anda tidak cocok!";
		} else {
			$sql = "UPDATE login SET password=md5('$_REQUEST[pass1]') WHERE login='$nip'";
			$result = QueryDb($sql);
			CloseDb();
...
if ($login=='landlord' || $login=='LANDLORD'){
	...
	$sql = "SELECT p.nip, p.nama FROM pegawai p WHERE p.nip = '$login'"; 
	$result = QueryDb($sql);

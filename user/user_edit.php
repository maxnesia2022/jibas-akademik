...
$sql="SELECT h.login, h.tingkat, h.departemen, h.keterangan, p.nama FROM hakakses h, pegawai p, login l WHERE h.modul='SIMAKA' AND h.login = l.login AND l.login = p.nip AND h.replid=$replid ";
$result = QueryDb($sql);
...
if (isset($_REQUEST['simpan'])) {
	...
	$sql = "SELECT * FROM hakakses WHERE login = '$_REQUEST[nip]' AND tingkat = $tingkat AND modul = 'SIMAKA' $sql_dep AND replid <> $replid";
	...
	if (mysqli_num_rows($result) > 0) {
		...
		if ($tingkat==1){
			//Kalo manajer
			$sql_hakakses="UPDATE hakakses SET tingkat=1, keterangan ='$keterangan' WHERE replid = $replid";
		} elseif ($tingkat==2){
			//Kalo staf
			$sql_hakakses="UPDATE hakakses SET departemen='$departemen', tingkat=2, keterangan ='$keterangan' WHERE replid = $replid";
		}

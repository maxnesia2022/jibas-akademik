...
if ($op == "xm8r389xemx23xb2378e23") {
	$sql = "DELETE FROM hakakses WHERE login = '$_REQUEST[login]' AND modul='SIMAKA'";
	QueryDb($sql);
	$sql = "SELECT * FROM hakakses WHERE login = '$_REQUEST[login]' AND modul<>'SIMAKA'";
	$result = QueryDb($sql);
	if (@mysqli_num_rows($result)==0) {
		$sql = "DELETE FROM login WHERE login='$_REQUEST[login]'";
		QueryDb($sql);
	}
	CloseDb();
}
...
<?
	OpenDb();
	$sql_tot = "SELECT h.login, h.replid,  h.tingkat, h.departemen, h.keterangan, p.nama, p.aktif,  DATE_FORMAT(h.lastlogin,'%Y-%m-%d') AS tanggal, TIME(h.lastlogin) as jam FROM hakakses h, pegawai p, login l WHERE h.modul='SIMAKA' AND h.login = l.login AND l.login = p.nip ";
	
	$result_tot = QueryDb($sql_tot);
	$total = ceil(mysqli_num_rows($result_tot)/(int)$varbaris);
	$jumlah = mysqli_num_rows($result_tot);
					
	$sql="SELECT h.login, h.replid,  h.tingkat, h.departemen, h.keterangan, p.nama, p.aktif,  DATE_FORMAT(h.lastlogin,'%Y-%m-%d') AS tanggal, TIME(h.lastlogin) as jam FROM hakakses h, pegawai p, login l WHERE h.modul='SIMAKA' AND h.login = l.login AND l.login = p.nip ORDER BY $urut $urutan LIMIT ".(int)$page*(int)$varbaris.",$varbaris";

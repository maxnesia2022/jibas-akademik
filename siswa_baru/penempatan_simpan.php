...
if (isset($_REQUEST['Simpan'])) 
{
	...
	else 
	{
		...
		BeginTrans();
		$success = true;		
		$sql = "INSERT INTO siswa SET nis='$nis',nama='$nama', panggilan='$row[panggilan]', tahunmasuk=$year, 
		...
		$sql_dept = "INSERT INTO riwayatdeptsiswa SET nis='$nis',departemen='$departemen',mulai='$kumplit'";
		if ($success)		
			QueryDbTrans($sql_dept,$success);
		
		$sql_kls = "INSERT INTO riwayatkelassiswa SET nis='$nis',idkelas='$kelas',mulai='$kumplit'";
		if ($success)
			QueryDbTrans($sql_kls,$success);

		if ($success)
        {
            $sql = "INSERT INTO tambahandatasiswa (nis, idtambahan, jenis, teks, filedata, filename, filemime, filesize)
                    SELECT '$nis', idtambahan, jenis, teks, filedata, filename, filemime, filesize
                      FROM tambahandatacalon

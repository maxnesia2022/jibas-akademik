...
if ($iddasar!="12"){
	if ($dasar == 'Golongan Darah') {
		...
		$query = "SELECT $tabel FROM kondisisiswa ORDER BY $tabel ";
		$result = QueryDb($query);
		$jum = @mysqli_num_rows($result);
	} elseif ($dasar == 'Status Siswa') {	
		$query = "SELECT $tabel FROM statussiswa ORDER BY $tabel ";
		$result = QueryDb($query);
		$jum = @mysqli_num_rows($result);
	} elseif ($dasar == 'Pekerjaan Ayah' || $dasar == 'Pekerjaan Ibu') {	
		$query = "SELECT pekerjaan FROM jenispekerjaan ORDER BY pekerjaan ";
		$result = QueryDb($query);
		$jum = @mysqli_num_rows($result)+1;		
	} elseif ($dasar == 'Pendidikan Ayah' || $dasar == 'Pendidikan Ibu') {	
		$query = "SELECT pendidikan FROM tingkatpendidikan ORDER BY pendidikan ";
		$result = QueryDb($query);
		$jum = @mysqli_num_rows($result)+1;
		...
		$query = "SELECT $tabel FROM $tabel ORDER BY $str";
		$result = QueryDb($query);
		$jum = @mysqli_num_rows($result);	
	} else {	
		$jum = 1;
	...
	for ($i=1;$i<=$jum;$i++) {	
		...
		if ($departemen=="-1" && $idproses<0) {	
			$query1 = "SELECT COUNT(*) As Jum$field FROM calonsiswa s, prosespenerimaansiswa a WHERE a.replid=s.idproses AND s.aktif = $filter"; 		
		}  
		if ($departemen<>"-1" && $idproses<0) {	
			$query1 = "SELECT COUNT(*) As Jum$field FROM calonsiswa s, prosespenerimaansiswa a WHERE a.departemen='$departemen' AND a.replid=s.idproses AND s.aktif = $filter";
		} 
		if ($departemen<>"-1" && $idproses>0) {	
			$query1 = "SELECT COUNT(*) As Jum$field FROM calonsiswa s, prosespenerimaansiswa a WHERE s.idproses='$idproses' AND a.replid=s.idproses AND a.departemen='$departemen' AND s.aktif = $filter ";
		}
	...
	$query1 = "SELECT COUNT(*) As Jum FROM calonsiswa s, prosespenerimaansiswa a WHERE a.replid=s.idproses  AND s.aktif = '1' AND s.penghasilanayah+s.penghasilanibu <> 0 AND s.penghasilanayah+s.penghasilanibu<1000000 $kondisi";
	...
	$query5 = "SELECT COUNT(*) As Jum FROM calonsiswa s, prosespenerimaansiswa a WHERE a.replid=s.idproses  AND s.aktif = '1' AND s.penghasilanayah+s.penghasilanibu = 0 $kondisi";
...
if ($idproses!="-1"){
	OpenDb();
	$sql_p="SELECT proses FROM prosespenerimaansiswa WHERE replid='$idproses'";
	$res_p=QueryDb($sql_p);

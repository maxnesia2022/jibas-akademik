...
if (isset($_REQUEST['simpan'])) {
	OpenDb();
	$sql_cek = "SELECT * FROM kelompokcalonsiswa WHERE kelompok = '".CQ($_REQUEST['kelompok'])."' AND idproses='$id' AND replid <> '$replid'";
	...
	if (mysqli_num_rows($result_cek) > 0) {
		...
		$sql = "UPDATE kelompokcalonsiswa SET kelompok='".CQ($_REQUEST['kelompok'])."', idproses='$id', kapasitas='$_REQUEST[kapasitas]', keterangan='".CQ($_REQUEST['keterangan'])."' WHERE replid='$replid'";
		$result = QueryDb($sql);

...
<head>
    ...
    <script type="text/javascript">
        var base_url = "proses.php";

        function tambah() {
            var departemen = document.getElementById('departemen').value;
            window.open('proses_add.php?departemen='+encodeURIComponent(departemen), 'TambahProsesPenerimaanSiswaBaru', 'width=500,height=327,resizable=1,scrollbars=1');
        }
        ...
        function edit(replid) {
            window.open('proses_edit.php?replid='+replid, 'UbahProsesPenerimaanSiswaBaru', 'width=500,height=327,resizable=1,scrollbars=1');
        }

        function hapus(replid) {
            var departemen = document.getElementById('departemen').value;
            if (confirm("Apakah anda yakin akan menghapus proses ini?"))
                window.location.href = base_url + "?op=xm8r389xemx23xb2378e23&replid="+replid+"&departemen="+encodeURIComponent(departemen)+"&urut=<?=$urut?>&urutan=<?=$urutan?>&page=<?=$page?>&hal=<?=$hal?>&varbaris=<?=$varbaris?>";
        }

        function cetak() {
            var departemen = document.getElementById('departemen').value;
            var total = document.getElementById("total").value;
            window.open('proses_cetak.php?departemen='+encodeURIComponent(departemen)+'&urut=<?=$urut?>&urutan=<?=$urutan?>&varbaris=<?=$varbaris?>&page=<?=$page?>&total='+total, 'CetakProsesPenerimaanSiswaBaru', 'width=790,height=650,resizable=1,scrollbars=1');
        }
...
<div class="max-w-7xl mx-auto space-y-4">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-emerald-100 p-5">
        ...
        <div class="flex justify-between items-center mb-4 border-b border-gray-100 pb-3">
            <h2 class="text-xl font-bold text-emerald-900 flex items-center gap-2">
                <i class="fas fa-door-open text-emerald-500"></i> Proses Penerimaan Siswa Baru
            </h2>
            <div class="text-sm text-gray-500 hidden sm:block">
                PPDB <i class="fas fa-chevron-right text-xs mx-1"></i> Proses Penerimaan
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-emerald-100 p-5">
        ...
        if ($departemen != "") {
            // Query Total
            $sql_tot = "SELECT COUNT(*) FROM prosespenerimaansiswa WHERE departemen='$departemen'";
            ...
            ?>
                ...
                <div class="text-sm text-gray-600 mb-4">
                    Ditemukan <span class="font-bold text-gray-900"><?=$jumlah?></span> proses penerimaan.
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        <thead class="bg-emerald-50">
                            <tr>        
                                ...
                                <th class="px-4 py-3 text-left text-xs font-bold text-emerald-800 uppercase tracking-wider cursor-pointer hover:bg-emerald-100 transition" onClick="change_urut('p.proses','<?=$urutan?>')">
                                    Proses <i class="fas fa-sort text-emerald-300 ml-1"></i>
                                </th>
                                <th class="px-4 py-3 text-left text-xs font-bold text-emerald-800 uppercase tracking-wider cursor-pointer hover:bg-emerald-100 transition" onClick="change_urut('p.kodeawalan','<?=$urutan?>')">
                                    Kode Awalan <i class="fas fa-sort text-emerald-300 ml-1"></i>
                                </th>
                                <th class="px-4 py-3 text-center text-xs font-bold text-emerald-800 uppercase tracking-wider w-24">Jumlah</th>
                                <th class="px-4 py-3 text-left text-xs font-bold text-emerald-800 uppercase tracking-wider">Keterangan</th>
                                <th class="px-4 py-3 text-center text-xs font-bold text-emerald-800 uppercase tracking-wider w-32 cursor-pointer hover:bg-emerald-100 transition" onClick="change_urut('p.aktif','<?=$urutan?>')">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-emerald-50 transition duration-150">                    
                                ...
                                <td class="px-4 py-2 text-gray-900 font-bold"><?=$row['proses']?></td>
                                <td class="px-4 py-2 text-emerald-700 font-mono"><?=$row['kodeawalan']?></td>
                                <td class="px-4 py-2 text-center">
                                    <span class="bg-gray-100 text-gray-700 py-1 px-2.5 rounded-lg font-bold text-xs">
                                        <?=$row1['jumlah']?>
                                    </span>
                                </td>
            ...
            <?php } else { ?>
                ...
                <div class="flex flex-col items-center justify-center p-16 text-center bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                    ...
                    <p class="text-gray-500 max-w-md mb-6">
                        Tidak ditemukan data proses penerimaan untuk departemen <strong><?=$departemen?></strong>.
                    </p>
                    <?php if (SI_USER_LEVEL() != $SI_USER_STAFF) { ?>

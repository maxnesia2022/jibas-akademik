...
if ($iddasar=="12"){	
	$query1 = "SELECT COUNT(*) As Jum FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi AND s.aktif = '1' AND s.penghasilanayah+s.penghasilanibu <> 0 AND s.penghasilanayah+s.penghasilanibu<1000000 ";

	$query2 = "SELECT COUNT(*) As Jum FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi AND s.aktif = '1' AND s.penghasilanayah+s.penghasilanibu>=1000000 AND s.penghasilanayah+s.penghasilanibu<2500000";

	$query3 = "SELECT COUNT(*) As Jum FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi AND s.aktif = '1' AND s.penghasilanayah+s.penghasilanibu>=2500000 AND s.penghasilanayah+s.penghasilanibu<5000000";

	$query4 = "SELECT COUNT(*) As Jum FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi AND s.aktif = '1' AND s.penghasilanayah+s.penghasilanibu>=5000000";

	$query5 = "SELECT COUNT(*) As Jum FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi AND s.aktif = '1' AND s.penghasilanayah+s.penghasilanibu = 0";
	...
	if ($iddasar=="1") {
		...
		$query1 = "SELECT COUNT(s.replid) As Jum, s.agama, g.replid FROM calonsiswa s, prosespenerimaansiswa a, agama g WHERE $kondisi AND s.aktif = 1 AND g.agama = s.agama GROUP BY s.agama ORDER BY g.agama";
	} if ($iddasar=="2") {
		$xaxis="Asal Sekolah";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Asal Sekolah";
		$query1 = "SELECT COUNT(s.replid) As Jum, s.asalsekolah FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi AND s.aktif = 1 GROUP BY s.asalsekolah";
	} if ($iddasar=="3"){
		$xaxis="Golongan Darah";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Golongan Darah";
		$query1 = "SELECT COUNT(s.replid) As Jum, s.darah FROM calonsiswa s, prosespenerimaansiswa a WHERE  $kondisi AND s.aktif = 1 GROUP BY s.darah";
	} if ($iddasar=="4"){
		$xaxis="Jenis Kelamin";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Jenis Kelamin";
		$query1 = "SELECT COUNT(s.replid) As Jum, s.kelamin FROM calonsiswa s, prosespenerimaansiswa a WHERE  $kondisi AND s.aktif = 1 GROUP BY s.kelamin";
	} if ($iddasar=="5"){
		$xaxis="Kewarganegaraan";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Kewarganegaraan";
		$query1 = "SELECT COUNT(s.replid) As Jum, s.warga FROM calonsiswa s, prosespenerimaansiswa a WHERE  $kondisi AND s.aktif = 1 GROUP BY s.warga ORDER BY s.warga DESC";
	} if ($iddasar=="6"){
		$xaxis="Kode Pos Siswa";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Kode Pos Siswa";
		$query1 = "SELECT COUNT(s.replid) As Jum, s.kodepossiswa FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi AND s.aktif = 1 GROUP BY s.kodepossiswa";
	} if ($iddasar=="7"){
		$xaxis="Kondisi";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Kondisi Siswa";
		$query1 = "SELECT COUNT(s.replid) As Jum, s.kondisi FROM calonsiswa s, prosespenerimaansiswa a WHERE  $kondisi AND s.aktif = 1 GROUP BY s.kondisi";
	} if ($iddasar=="8"){
		$xaxis="Jenis Pekerjaan Ayah";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Pekerjaan Ayah";
		$query1 = "SELECT COUNT(s.replid) As Jum, s.pekerjaanayah FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi AND s.aktif = 1 GROUP BY s.pekerjaanayah";
	} if ($iddasar=="9"){
		$xaxis="Jenis Pekerjaan Ibu";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Pekerjaan Ibu";
		$query1 = "SELECT COUNT(s.replid) As Jum, s.pekerjaanibu FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi AND s.aktif = 1 GROUP BY s.pekerjaanibu";
	} if ($iddasar=="10"){
		$xaxis="Tingkat Pendidikan Ayah";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Pendidikan Ayah";
		$query1 = "SELECT COUNT(s.replid) As Jum, s.pendidikanayah FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi AND s.aktif = 1 GROUP BY s.pendidikanayah";
	} if ($iddasar=="11"){
		$xaxis="Tingkat Pendidikan Ibu";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Pendidikan Ibu";
		$query1 = "SELECT COUNT(s.replid) As Jum, s.pendidikanibu FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi AND s.aktif = 1 GROUP BY s.pendidikanibu";
	} if ($iddasar=="13"){
		$xaxis="Status Aktif";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Berdasarkan Status Aktif";
		$query1 = "SELECT COUNT(s.replid) As Jum, s.aktif FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi GROUP BY s.aktif";
	} if ($iddasar=="14"){
		$xaxis="Status Siswa";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Status Siswa";
		$query1 = "SELECT COUNT(s.replid) As Jum, s.status FROM calonsiswa s, prosespenerimaansiswa a WHERE  $kondisi AND s.aktif = 1 GROUP BY s.status";
	} if ($iddasar=="15"){
		$xaxis="Suku";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Suku";
		$query1 = "SELECT COUNT(s.replid) As Jum, s.suku FROM calonsiswa s, prosespenerimaansiswa a WHERE  $kondisi AND s.aktif = 1 GROUP BY s.suku";
	} if ($iddasar=="16"){
		$xaxis="Tahun Kelahiran";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Tahun Kelahiran";
		$query1 = "SELECT COUNT(s.replid) As Jum, YEAR(s.tgllahir) as thnlahir FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi AND s.aktif = 1 GROUP BY thnlahir";
	} if ($iddasar=="17"){
		$xaxis="Usia";
		$yaxis="Jumlah Calon Siswa";
		$titlenya="Statistik Calon Siswa Aktif Berdasarkan Usia";
		$query1 = "SELECT COUNT(s.replid) As Jum, YEAR(now())-YEAR(s.tgllahir) As usia FROM calonsiswa s, prosespenerimaansiswa a WHERE $kondisi AND s.aktif = 1 GROUP BY usia";
	}

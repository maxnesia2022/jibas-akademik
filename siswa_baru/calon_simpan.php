...
$sql = "SELECT kodeawalan FROM prosespenerimaansiswa WHERE replid = '$proses'";	
$res = QueryDb($sql);	
...
$sql = "SELECT MAX(LPAD(nopendaftaran, " . ($kodelen + 20) . ",'*')) FROM calonsiswa WHERE idproses = '$proses'";
$res = QueryDb($sql);	
$row = mysqli_fetch_row($res);
$nom = $row[0];
//echo "$nom<br>";
...
do
{
    ...
    $sql = "SELECT COUNT(replid) FROM calonsiswa WHERE nopendaftaran='$no'";
    //echo "$sql<br>";
    $res = QueryDb($sql);
    $row = mysqli_fetch_row($res);
    $ndata = (int)$row[0];
...
else 
{
	...
	{
	    $sql = "SELECT isnull(foto) FROM calonsiswa WHERE replid='$_REQUEST[replid]'";
	    $res2 = QueryDb($sql);
	    ...
	    if ($isFotoNull == 0)
	    {
	        $sql = "SELECT foto FROM calonsiswa WHERE replid='$_REQUEST[replid]'";
	        $res2 = QueryDb($sql);
	        $row2 = mysqli_fetch_row($res2);
	        $data = $row2[0];
...
if ($success)
{
    if ($uploadedfile != "")
    {
        $sql = "INSERT INTO riwayatfoto SET nic = '$no', foto = '$foto_data', tanggal = NOW()";
        QueryDbTrans($sql, $success);
    }
}
...
if ($success && strlen($idtambahan) > 0)
{
    ...
    for($i = 0; $success && $i < count($arridtambahan); $i++)
    {
        ...
        if ($jenis == 1 || $jenis == 3)
        {
            ...
            if ($repliddata == 0)
                $sql = "INSERT INTO tambahandatacalon
                           SET nopendaftaran = '$no', idtambahan = '$replid', jenis = '$jenis', teks = '$teks'";
            else
                $sql = "UPDATE tambahandatacalon
                           SET teks = '$teks'

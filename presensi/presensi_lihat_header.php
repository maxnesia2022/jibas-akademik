...
if ($kelas <> "" && $semester <> "" && $tingkat <> "") { 
	OpenDb();
	$sql = "SELECT DAY(pp.tanggal), MONTH(pp.tanggal), pp.jam, p.nama, g.nama, pp.materi, pp.replid FROM presensipelajaran pp, pelajaran p, pegawai g WHERE pp.idkelas = '$kelas' AND pp.idsemester = '$semester' '$pel' AND MONTH(pp.tanggal) = '$bln' AND YEAR(pp.tanggal) = '$thn' AND pp.idpelajaran = p.replid AND pp.gurupelajaran = g.nip ORDER BY pp.tanggal, pp.jam ";
